...
Salesforce is the sole Account system of record. Account data must not be copied into AWS databases, logs, traces, analytics, caches, service-worker storage, fixtures derived from production, or source control.

## Trust boundaries

- Browser to Cognito Managed Login.
- Browser through CloudFront to API Gateway.
...
The shared Salesforce integration user means every admitted application user has the same Salesforce capabilities exposed by the API. Production use requires explicit data-owner acceptance, controlled Cognito provisioning, MFA, and least-privileged Salesforce sharing/object/field permissions.
...
- Fixed SOQL and strict allowlisted DTOs; no dynamic Salesforce query inputs.
